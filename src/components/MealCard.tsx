    ...
    <section className="card flex flex-col p-4">
      ...
      <footer className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-sm">
        <button
          ...
          disabled={!hasItems}
          className="text-xs font-medium text-brand-600 hover:text-brand-700 disabled:cursor-not-allowed disabled:text-slate-400"
          title={hasItems ? 'Save current items as a reusable template' : 'Log some foods first'}
        >
          💾 Save as template
        </button>
        <span className="text-slate-700">
          <strong className="font-semibold text-calorie-600">
            {formatNumber(totals.calories)} kcal
          </strong>
          <span className="mx-1.5 text-slate-300">|</span>
          <strong className="font-semibold text-protein-600">
